...
from ColBERT_AM.data import Collection, Queries, Ranking

from ColBERT_AM.modeling.checkpoint import Checkpoint
from ColBERT_AM.search.index_storage import IndexScorer

from ColBERT_AM.infra.provenance import Provenance
from ColBERT_AM.infra.run import Run
from ColBERT_AM.infra.config import ColBERTConfig, RunConfig
from ColBERT_AM.infra.launcher import print_memory_stats
...
class Searcher:
    def __init__(self, index, checkpoint=None, collection=None, config=None, index_root=None, verbose: int = 3):
        ...
        # Merge all configs (but `collection` is missing)
        ...
        # Manually set `collection`
        self.config.collection = collection or config.collection  

        # Debugging: Ensure `collection` is correctly assigned
        print(f"DEBUG: Final collection path -> {self.config.collection}")

        if self.config.collection is None:
            raise ValueError("`collection` is still None inside Searcher! Check config and indexing.")
