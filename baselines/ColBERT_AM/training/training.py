...
import numpy as np
import torch.distributed as dist  # Added for distributed training
from transformers import get_linear_schedule_with_warmup
from torch.optim import AdamW  # Instead of transformers' AdamW

from ColBERT_AM.infra import ColBERTConfig
from ColBERT_AM.training.rerank_batcher import RerankBatcher
from ColBERT_AM.utils.amp import MixedPrecisionManager
from ColBERT_AM.training.lazy_batcher import LazyBatcher
from ColBERT_AM.parameters import DEVICE
from ColBERT_AM.modeling.colbert import AmharicColBERT
from ColBERT_AM.modeling.reranker.electra import ElectraReranker
from ColBERT_AM.utils.utils import print_message
from ColBERT_AM.training.utils import print_progress, manage_checkpoints


# def train(config: ColBERTConfig, triples, queries=None, collection=None):
def train(config, triples, queries, collection, savepath=None):
    ...
    if not config.reranker:
        colbert = AmharicColBERT(name=config.checkpoint, colbert_config=config)
    else:
    ...
    if torch.cuda.device_count() > 1:
        ...
        colbert = colbert.to(DEVICE)  # Standard single-GPU training
